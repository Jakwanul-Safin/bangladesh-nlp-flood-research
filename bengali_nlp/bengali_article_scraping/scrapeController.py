...
class ScrapeController:
    ...
    def __init__(self, links, output_file, 
                 skip_articles = set(), 
        ...
        """" 
        @param links - [{'url': <URL>, 'paper': <paper-name>, (optionally has 'title': <title>)}]
    ...
    def run(self):
        ...
        with open(self.output_file, 'a+', newline='', encoding="utf-8") as fullArticleFile:
            ...
            totalArticles = sum(len(links) for paper, links in self.links.items())
            logger.info(f"Found {totalArticles} links")
            
            scrapeLinks = {paper: [link for _, link in links.iterrows()
