    ...
    while next_page != -1:
        ...
        if json_string:
            ...
            for doc in json_dict["docs"]:
                ...
                response = requests.get(args.req_url, params=args.req_params, auth=args.req_auth, verify=args.verify_ssl)
                args.req_params["output"] = 'text'
                responseText = requests.get(args.req_url, params=args.req_params, auth=args.req_auth, verify=args.verify_ssl)
                args.req_params["output"] = 'html'
                responseHTML = requests.get(args.req_url, params=args.req_params, auth=args.req_auth, verify=args.verify_ssl)
                args.req_params["output"] = download_output
                ...
                if response.ok:
                    ...
                    js['filename'] = doc['filename']
                    if responseText: js['text'] = responseText.text
                    else: js['text']=None
                    if responseHTML: js['html'] = responseHTML.text
                    else: js['html']=None
                    json.dump(js, open(filepath, 'w'), indent=2)
