...
def get_new_predicted_data(prediction_folder='predictions'):
    to_remove_columns = ['abstract', 'news_keywords', 'description', 'keywords', 'dateModified',
                     'link', 'query_info', 'headline', 'authors', 'id']
...
def complete_day(list1, year_range=[2015, None], aggType=None, normDict=None, normAgg='month', ignoreDates = {}):
    ...
    for i in all_years:
            ...
            if normDict is not None:
                ...
                if normAgg == 'month':
                    if normFactorAv[0] == 0:
                        ...
                        innerArr = [(k,n/(normFactorAv[0] / normFactorAv[1])) for k,n in innerArr]
        ...
        if normAgg == 'year':
            innerMonth = [(k,n/(count / 365)) for k,n in innerMonth]
...
def plot_2timeseries(l1, l2, title='', xlabel='', xticks_rotate=False, skip=0, xticksFontSize=16):
    plt.figure(figsize=(15,7))
    ...
    ax1.set_ylabel(l1.get('ylabel', ''), color=plot_color, fontsize=14)
    ax1.tick_params(axis='y', labelcolor=plot_color, labelsize=14)
    ...
    ax2.set_ylabel(l2.get('ylabel', ''), color=plot_color, fontsize=14)
    ax2.tick_params(axis='y', labelcolor=plot_color, labelsize=14)
...
def get_Sentinel1_data_monthly(division, agg_func=max, div_factor=1000000, ignoreDates={}):
    ...
    s1_df_list = [(row[1]['Dates'], row[1]['Flooded Area (m^2)']/div_factor) for row in s1_df.iterrows()]
